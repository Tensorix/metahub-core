...
新表 `doc_blocks(id, doc_id, text, order_key, __deleted)`,每个字段是独立 CRDT register(复用现有 `emit/applyChange/ingest`,只在 `crdt.ts` 的 `DOMAIN` 白名单里登记)。正文 = 该文档未删块按 `ORDER BY order_key, id` 拼接、以 `\n\n` 连接。
