...
- **同步循环**(worker 内):启动 / online / visibilitychange / 变更后防抖 800ms 触发同步;origin-backed 反应式副本不做前台 15s 轮询(见 20 §5),但直连桶时会把“本地已写入、尚未落桶”的状态暴露给 UI,右上角“分享”动画切为“保存”,点击或页面隐藏会 force flush,小尾巴也有短延迟自动落桶。每轮 pull 后从本地 oplog 高水位差分出 `synced` 事件(零 core 改动)。编辑器收到命中 documents/doc_blocks 的 `synced` → flush 未保存键入 → 本地重读 → version 没变即跳过,变了原位合并刷新(本地路径**不带 if_match**——单写者不自我竞争,stale 是 HTTP 模式概念);表格视图同理(单元格编辑中跳过)。
