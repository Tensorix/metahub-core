...
当前系统已经具备基础 typed table、Markdown 文档、全文检索、修订历史与回滚(实体粒度,含 schema 回滚与 oplog 压缩)、快照恢复、多设备同步、对外发布与访客写入、以及"数据在哪/谁碰得到/丢了怎么办"的信任面。对于记账和 IM 历史消息场景,目前还处于“可手动建表 + 基础查询”的 MVP 状态,尚未形成领域化模板、批量导入、聚合、上下文检索等完整流程。
