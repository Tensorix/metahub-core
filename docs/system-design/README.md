...
- 提供 CLI、库入口和独立二进制三种分发形态;同步服务端(`mh --server`)在根路径内置浏览器 WebUI + `/api/*` REST 接口。
- 实体 id 带类型前缀且可用前缀/名字引用,配合当前库上下文(`mh use`)与 Tab 补全,降低人/AI 的 id 心智负担。

当前系统已经具备基础 typed table、Markdown 文档、全文检索、修订历史与回滚(实体粒度,含 schema 回滚与 oplog 压缩)、快照恢复和多设备同步能力。对于记账和 IM 历史消息场景,目前还处于“可手动建表 + 基础查询”的 MVP 状态,尚未形成领域化模板、批量导入、聚合、上下文检索等完整流程。
