...
- Markdown 快捷输入：`# ` / `## ` / `### ` 转标题，`- ` / `* ` / `+ ` 转无序列表，`1. ` / `5. ` 转有序列表（按输入数字定起始号），`- [ ] ` / `- [x] ` 转待办，`> ` 转引用，```` ``` ```` 或 ```` ```python ```` + Enter 转代码块并保留语言名。
- 列表行为：Enter 续同级列表，空列表项 Enter 退出列表；Tab/Shift+Tab 缩进/反缩进形成嵌套。列表项内可包含段落、引用、代码块和子列表。有序列表序号由首项起算、后续自动递增，插入/删除/重排后自动重建。
