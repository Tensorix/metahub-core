...
当一个 AI agent 直接驱动 CLI 写你的库时,「刚才它到底动了什么」不再需要逐个实体去翻历史:

### 1. 看全局变更流

```bash
mh audit                       # 整个库的每一次修改,新→旧;每条一行:时间 / actor / 类型 / 摘要 / txn
mh audit list --actor ai       # 只看 agent 干的(非 TTY 运行的 CLI 自动带 ai 标)
mh audit list --limit 20 --before <cursor>   # 翻更早的一页
```

### 2. 展开一条,确认是不是要撤

```bash
mh audit show <txn>            # 逐实体、逐字段 before → after
```

### 3. 整组撤销

```bash
mh audit revert <txn>          # 按逻辑变更组撤销;输出 restored/removed/skipped 计数
```

- 回滚是**正向写入**,随 sync 收敛,自身还能再回滚。
- 你或别的设备在那之后改过的寄存器/行会被**保留**并计入 `skipped_*`——撤销不会顺手抹掉后来的工作。
- 深度受压缩窗口约束:`mh compact` 窗口之外的条目已折叠,不再可展开或回滚。
- 想让某次脚本运行**不被**记成 `ai`,设 `MH_ACTOR=human`(或空串);想换个标签就设成别的短 slug(如 `MH_ACTOR=nightly`)。
- WebUI:设置 → 工作区 →「操作审计」是同一份流,带 actor 过滤与一键回滚。
...
- 侧栏顶部是三个标签页:**文档 / 数据表 / 站点**(`⌘1`–`⌘3`,滑动指示条)。文档为**树**，可折叠、拖拽改嵌套与排序（drop-into 嵌为子页、before/after 在同级间重排，均经 `PATCH /api/document/move` 持久化——`parent_id` 与 `order_key` 一次原子更新）。
- 悬停条目出现「+ 子页」与 ⋯ 菜单（重命名/复制/删除/移到顶层），右键同样打开该菜单。「数据表」标签的「+」弹**新建数据库 Modal**（名称 + 图标 + 模板：空白/任务/联系人）。侧栏宽度可拖拽、整栏可 `⌘\` 收起；窄屏（≤768px）变抽屉。
- 列表加载时显示**真实行形状的骨架**(记住上次行数),不写「加载中…」。

### 3. 表格（Notion-like）

- 点选数据库 → 按属性渲染网格。单元格编辑：checkbox 即时切、select/multi_select 弹彩色 chip 菜单、relation/doc 弹记录选择器；其余类型（text/number/date/url）双击或选中后直接打字进入**覆盖式编辑器**——悬浮于单元格上方、行高不变；点击别处/Enter/Tab 均提交、Esc 放弃、值不变不发请求；乐观更新本地即时生效，后台 `PATCH /api/record`，失败 toast+回滚。中文输入法 Enter 选词不会误提交。
...
- **列头菜单**：改名、**改类型**（`PATCH /api/property`，改类型会清空该列单元格）、select 选项管理(就地改名 / 拖拽排序 / 删除——改名与删除经 core **级联重写**已有单元格)、排序、在右侧插入列、删除列。末列「+」按类型新建属性;选 relation 会进入第二步**挑目标库**。
- **关联列**(`relation` / `doc`)显示目标标题胶囊,点开是记录选择器(搜索 / 新建 / 多选);胶囊点击跳 `#/db/<db>/<rec>` 深链打开对方的 peek。
...
- 侧栏「站点」标签 → 点进去是**沉浸式访问页**(`#/site/<name>`,浮动导航、无顶栏),右上进入**配置页**(`?view=config`:身份卡 + 缩略预览 + 改名/可见性/SPA/文件管理/发布渠道)。
- 顶栏「分享」把当前文档/数据库/站点发布成能力链接;全局「分享」视图列出**所有**可见分享(本机 + 每个挂载桶 + 每个已配对 peer),一行一个主操作(复制 / 续期 / 重新分享 / 重试撤销),按状态与来源过滤并带计数,过期项沉到独立分组可批量清理。

### 7. 设置(两组 + 无头组、九页)

- **设备**组:外观 / 快速笔记(仅桌面) / 离线与缓存(桌面隐藏);**工作区**组:数据与备份 / 设备 / **操作审计** / 站点与发布;末尾无头组:**快捷键**速查表与**关于**(版本、检查更新)。
- 「数据与备份」顶部就是**数据地图**一句话结论(数据在几处、有没有只在本机的附件),下面是桶接入、换钥、恢复码;「设备」是名册(怎么加入的、最后活跃、能否吊销;吊销一台设备一次性作用于它的全部凭据);「操作审计」是全库变更流 + AI 过滤 + 一键回滚;「站点与发布」管托管目标与入口验证。
...
- 页面挂着 `GET /api/changes` 的实时流:在终端跑 `mh record update` / `mh doc edit`,**打开着的视图会自己更新**(1~2 秒),不必手动刷新。
